...
function done(box){box.find(overlay).remove();settings.onLoadDone.call(box);}};})(jQuery);(function($){'use strict';$.fn.activateBox=function(){$.AdminLTE.boxWidget.activate(this);};})(jQuery);(function($){'use strict';$.fn.todolist=function(options){var settings=$.extend({onCheck:function(ele){return ele;},onUncheck:function(ele){return ele;}},options);return this.each(function(){if(typeof $.fn.iCheck!='undefined'){$('input',this).on('ifChecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onCheck.call(ele);});$('input',this).on('ifUnchecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onUncheck.call(ele);});}else{$('input',this).on('change',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");if($('input',ele).is(":checked")){settings.onCheck.call(ele);}else{settings.onUncheck.call(ele);}});}});};}(jQuery));function add_msg(msg,context,type){context=typeof context!=='undefined'?context:'Info';type=typeof type!=='undefined'?type:'info';new PNotify({text:'<strong>Galer&iacute;a:</strong> '+msg,type:type,icon:false});}
function load_zoom_img(){$.magnificPopup.instance.popupsCache={};$('.fancybox').magnificPopup({type:'image',removalDelay:500,callbacks:{beforeOpen:function(){this.st.image.markup=this.st.image.markup.replace('mfp-figure','mfp-figure mfp-with-anim');this.st.mainClass='mfp-move-horizontal';}},closeOnContentClick:true,midClick:true});}
function set_breadcrumb(){$('ol#breadcrumb').html('<li><a href="">Inicio</a></li>');var parts=PATH.split('/');var html='';var link='';for(var i=1;i<parts.length;i++){if(i!=parts.length-1){link+=i==0?parts[i]:'/'+parts[i];html+='<li><a href="'+link+'">'+parts[i]+'</a></li>';}else{html+='<li class="active">'+parts[i]+'</li>';}}
$('ol#breadcrumb').append(html);$('ol#breadcrumb a').click(function(e){e.preventDefault();browse($(e.target).attr('href'));});}
function browse(path){path=typeof path!=='undefined'?path:'';$.ajax({url:pathListFilemanager,data:{data:path},cache:false,dataType:'json',type:'POST',success:function(result){if(result.status)
show_content(path,result.files);else
add_msg(result.msg,'PHP','error');},error:function(jqXHR,status){add_msg(status,'AJAX','error');},complete:function(){loader.hide();load_zoom_img();$('#gallerybrowser').modal('show');}});}
function show_content(path,files){PATH=path;set_breadcrumb();$('table#filemanager').empty();for(var i=0;i<files.length;i++){var f=files[i];if(f.folder){f.icon='carpeta';f.name=$('<a />').attr('href',f.link).text(f.name).click(function(e){e.preventDefault();browse($(this).attr('href'));});}else{var extension=f.name.substr((f.name.lastIndexOf('.')+1));if(f.thumb=='pdf'){f.icon=$('<i class="fa fa-file-pdf-o"></i>');}else if(f.thumb=='doc'||f.thumb=='docx'){f.icon=$('<i class="fa fa-file-word-o"></i>');}else if(f.thumb=='xls'||f.thumb=='xlsx'){f.icon=$('<i class="fa fa-file-excel-o"></i>');}else if(f.thumb=='ppt'||f.thumb=='pptx'){f.icon=$('<i class="fa fa-file-powerpoint-o"></i>');}else if(extension=='gif'||extension=='png'||extension=='jpg'||extension=='jpeg'){f.icon=$('<a class="fancybox"/>').attr('href',asset_url+f.link).append($('<img src="'+asset_url+f.thumb+'" />'));}
f.name=$('<a />').attr('href',f.link).text(f.name).click(function(e){e.preventDefault();var imagen=$(this).attr('href'),retornar=$('table#filemanager').attr('data-retornar'),file=$(this).text(),extension=file.substr((file.lastIndexOf('.')+1));$('#'+retornar).attr('value',imagen);$('#t_'+retornar).find('.fancybox').attr('href',asset_url+imagen);$('#t_'+retornar).find('.nombre-img').text(imagen.substr(8));$('#t_'+retornar).find('.trash, .nombre-img').removeClass('hide');if(extension=='gif'||extension=='png'||extension=='jpg'||extension=='jpeg'){$('#t_'+retornar).find('.fancybox').removeClass('hide');}else{$('#t_'+retornar).find('.fancybox').addClass('hide');}
$('#gallerybrowser').modal('hide');unsaved=true;});}
f.remove=$('<a class="btn btn-default btn-xs" />').attr('href',f.link).html('<i class="fa fa-trash"></i>').click(function(e){e.preventDefault();$('div#remove input#remove-path').val($(this).attr('href'));$('div#remove').modal('show');});$('table#filemanager').append($('<tr />').append($('<td class="text-center" />').append(f.icon),$('<td />').append(f.name).append((f.medidas=='')?'':'<p class="help-block">'+f.medidas+'</p>'),$('<td class="hidden-xs"/>').text(f.size),$('<td class="hidden-xs"/>').text(f.date),$('<td style="text-align:right">').append(f.remove)));}}
function progress_bar(e){var done=e.position||e.loaded;var total=e.totalSize||e.total;var per=(Math.floor(done/total*1000)/10);$('div#progress > div.bar').css('width',per+'%').text(per+' %');}
$('div#new form').submit(function(e){$('div#new').modal('hide');$.ajax({url:pathCreateFolderFilemanager,data:{data:$('div#new input#new-path').attr('data-base')+$('div#new input#new-path').val()},cache:false,dataType:'json',type:'POST',success:function(result){add_msg(result.msg,'PHP',result.status?'success':'error');browse(PATH);},error:function(jqXHR,status){add_msg(status,'AJAX','error');}});return false;});$('div#remove a.submit').click(function(e){$.ajax({url:pathDeleteFilemanager,data:{data:$('div#remove input#remove-path').val()},cache:false,dataType:'json',type:'POST',success:function(result){add_msg(result.msg,'PHP',result.status?'success':'error');browse(PATH);},error:function(jqXHR,status){add_msg(status,'AJAX','error');}});});$('div#upload').on('hidden.bs.modal',function(e){myDropzone.removeAllFiles();browse(PATH);if(!$('body').hasClass('modal-open')){$('body').addClass('modal-open');}});$('div#tools a#upload-button').click(function(e){e.preventDefault();$('div#upload input#upload-path').val((PATH==''?'':(PATH+'/')));$('div#upload').modal('show');});$('div#tools a#new-folder-button').click(function(e){e.preventDefault();$('div#new input#new-type').val('folder');$('div#new input#new-path').attr('data-base',PATH==''?'':(PATH+'/'));$('div#new').modal('show');});$('div#new').on('shown.bs.modal',function(e){$('#new-path').focus();});$('div#new').on('hidden.bs.modal',function(e){$('div#new input#new-path').val('');$('div#new input#new-path').attr('data-base','');});$('div#tools a#refresh-button').click(function(e){browse(PATH);});PNotify.prototype.options.styling="bootstrap3";PNotify.prototype.options.styling="fontawesome";$win=$(window);var wpos,space;$footer=$('#form-bloque-botones');function h(){var ipos=parseInt($footer.offset().top);wpos=$win.scrollTop();space=$win.height()-$footer.height()*4;if(wpos+space<(ipos-250)){$footer.addClass('fixed_footer');}else{$footer.removeClass('fixed_footer');}}
$('body').on('click','.help-block a',function(event){event.preventDefault();var url=$(this).attr('href');window.open(url,'_blank');});$('body').loadingIndicator();var loader=$('body').data("loadingIndicator");$.ajaxSetup({beforeSend:function(){loader.show();},complete:function(){loader.hide();load_zoom_img();}});function loadContent(href,param){param=typeof param!=='undefined'?param:false;$('#main-content').find("#guts").fadeOut(200,function(){$('#main-content').hide().load(href+'?ajax=true',param,function(){$('#main-content').fadeIn(200);$('.sidebar a').parent().removeClass('active');if($('.sidebar a').parent().parent().hasClass('treeview-menu')){$('.sidebar a').parent().parent().parent().removeClass('active');}
var path=href.split('/'),last=path[path.length-1];if($.isNumeric(last)||last=='new'){path[path.length-1]='';}
var activate=$('.sidebar a[href="'+path.join('/')+'"]');activate.parent().addClass('active');if(activate.parent().parent().hasClass('treeview-menu')){activate.parent().parent().parent().addClass('active');}});});}
function getConfirm(confirmMessage,callback){confirmMessage=confirmMessage||'';$('#confirmbox').modal({show:true,backdrop:false,keyboard:false,});$('#confirmFalse').click(function(){$('#confirmbox').modal('hide');if(callback)callback(false);});$('#confirmTrue').click(function(){$('#confirmbox').modal('hide');if(callback)callback(true);});}
function update_collection_order(){$('ul.collection').each(function(idx){var bloque=$(this).attr('id');$('#'+bloque+' > li.media').each(function(idx){actual=$(this).attr('id').replace('campo-','');var separar=actual.split('_'),numero_actual=separar[separar.length-1];separar[separar.length-1]=idx;var nuevo=separar.join('_');$(this).find('[id*="'+actual+'"]').each(function(){var interno=$(this).attr('id');$(this).attr('id',interno.replace(actual,nuevo));});$(this).find('[data-return*="'+actual+'"]').each(function(){var interno=$(this).attr('data-return');$(this).attr('data-return',interno.replace(actual,nuevo));});$(this).find('[for*="'+actual+'"]').each(function(){var interno=$(this).attr('for');$(this).attr('for',interno.replace(actual,nuevo));});$(this).find('[name*="['+numero_actual+']"]').each(function(){var interno=$(this).attr('name');$(this).attr('name',interno.replace('['+numero_actual+']','['+idx+']'));});$(this).attr('id','campo-'+nuevo);});});}
function id_existe_collection(name,id){var idname_1='#t_'+name+'_'+id;var idname_2='#'+name+'_'+id;if($(idname_1).length==0&&$(idname_2).length==0){return id;}else{return id_existe_collection(name,(parseInt(id)+1));}}
function load_sortable(){$('.collection').sortable({handle:'.mover',onDragStart:function($item,container,_super){if(!container.options.drop)
$item.clone().insertAfter($item);_super($item,container);},afterMove:function($placeholder,container,$closestItemOrContainer)
{}});}
function destroy_sortable(){$(".collection").sortable("destroy");$(".collection li").removeClass('ui-state-default');}
Dropzone.autoDiscover=false;var myDropzone=new Dropzone("form#my-awesome-dropzone",{url:url_upload});if(CKEDITOR.env.ie&&CKEDITOR.env.version<9)
CKEDITOR.tools.enableHtml5Elements(document);CKEDITOR.config.toolbar=[{name:'document',items:['Source','-','NewPage','Preview','-','Templates']},['Cut','Copy','Paste','PasteText','PasteFromWord','-','Undo','Redo'],'/',{name:'basicstyles',items:['Bold','Italic','-','Subscript','Superscript','-','RemoveFormat']},{name:'paragraph',groups:['list','indent','blocks','align','bidi'],items:['NumberedList','BulletedList','-','Outdent','Indent','-','JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock']},{name:'links',items:['Link','Unlink']},{name:'insert',items:['Table','Youtube']}];$(document).on('click','.media-right .eliminar',function(){$(this).parent().parent().remove();formHasChanged=true;});$(document).on('change','.box .form-group input, .box .form-group select, .box .form-group textarea',function(e){unsaved=true;});$(document).on('click','#main-content .collection-add',function(e){e.preventDefault();var id_actual=$(this).parent().attr('id');total=$('#field-'+id_actual+' > li.media').length,newWidget=$(this).parent().attr('data-prototype');newWidget=newWidget.replace(/__opt_prot__/g,total);var newLi=$('<li class="media"></li>').attr('id','campo-'+id_actual+'_'+total).append($('<div class="media-body"></div>').html(newWidget)).append($('<div class="media-right">'+'<button class="close eliminar" type="button"><i class="fa fa-trash-o"></i></button>'+'<button class="close mover" type="button"><i class="fa fa-arrows"></i></button>'+'</div>'));newLi.appendTo('#field-'+id_actual);unsaved=true;});$(document).on('click','#main-content .media .trash',function(){var retornar=$(this).attr('data-id');$('#'+retornar).attr('value','');$('#t_'+retornar).find('.fancybox, .trash, .nombre-img').addClass('hide');formHasChanged=true;});function CKupdate(){for(instance in CKEDITOR.instances)
CKEDITOR.instances[instance].updateElement();}
window.addEventListener('popstate',function(){loadContent('');});$(document).ready(function(){load_sortable();loader.hide();load_zoom_img();$(document).on('submit','#form-zero',function(){return false;});$(document).on('submit','.modalnoajax form',function(){return false;});$(document).on('click','#FORM_DELETE button[type="submit"]',function(event){unsaved=false;if(Modernizr.history){var form_data=$('#FORM_DELETE form').serializeArray();form_data.push({'name':$(this).attr('name')});loadContent($('#FORM_DELETE form').attr('action'),form_data);}
$('#FORM_DELETE').modal('hide');});$(document).on('click','#form-bloque-botones button[type="submit"]',function(){update_collection_order();CKupdate();unsaved=false;if(Modernizr.history){var form_data=$('#form-zero').serializeArray();form_data.push({'name':$(this).attr('name')});loadContent($('#form-zero').attr('action'),form_data);}});window.onbeforeunload=function(e){if(unsaved){var message=msg_warning_form_no_saved.replace('<br>','\n'),e=e||window.event;if(e){e.returnValue=message;}
return message;}}
$(document).on('click','.open-filebrowser',function(e){e.preventDefault();var retornar=$(this).attr('data-return');$('table#filemanager').attr('data-retornar',retornar);var ruta=$('#'+retornar).val().replace('uploads','').split('/');ruta.splice(-1,1);ruta=ruta.join('/');browse(ruta);});var link_cp=$('#FOSChangePasswordButton'),link_cp_href=link_cp.attr('href'),modal_cp=$('#FOSChangePassword');link_cp.click(function(e){e.preventDefault();modal_cp.find('.modal-content').load(link_cp_href,function(){modal_cp.modal('show');});});modal_cp.on('submit','form',function(e){e.preventDefault()
var form_data=modal_cp.find('form').serializeArray();modal_cp.find('.modal-content').load(link_cp_href,form_data,function(response,status,xhr){if(response=='correcto'){modal_cp.modal('hide');new PNotify({text:'Contrase  a actualizada',type:'success',icon:false});}});return false;});if(Modernizr.history){$(document).on('click','.control-ajax a',function(){var _link=$(this).attr("href");if($(this).hasClass('no-ajax')){return true;}
if(_link!='#'){if(unsaved){getConfirm(msg_warning_form_no_saved,function(result){if(result){history.pushState(null,null,_link);loadContent(_link,false);unsaved=false;}});}else{history.pushState(null,null,_link);loadContent(_link,false);}
return false;}
return true;});}});